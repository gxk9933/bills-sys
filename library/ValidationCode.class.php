...
class ValidationCode {
    ...
    function __construct ($width = 65, $height = 24, $codeNum = 4) {
    ...
    private function getCreateImage () { //用来创建图像资源，并初使化背影
        ...
        $border = imageColorAllocate($this->image, 171, 173, 179);
        imageRectangle($this->image, 0, 0, $this->width - 1, $this->height - 1, 
        $border);
    }
...
//             $number = rand(0, 2);
    ...
    private function outputText () { //随机颜色、随机摆放、随机字符串向图像中输出
        for ($i = 0; $i <= $this->codeNum; $i ++) {
            $bg_color = imagecolorallocate($this->image, rand(0, 255), 
    ...
    function outputImage () { //自动检测GD支持的图像类型，并输出图像
